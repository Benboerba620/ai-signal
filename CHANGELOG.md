...
- 修复 9/22 起每日 feed 生成全部中断的问题：9/21 撤除官方频道后，一条自检仍要求这五个频道存在，导致每次运行在抓取前就失败，播客、博客、论文 feed 停在 9/21。自检已改为确认这五个频道不在订阅中。
...
- 三档播客开启本地转录复用试点：通过独立发布名单和公开 RSS 身份核验，将已有转录提交至单集收件目录，由云端统一合并。保留云端发现与缺失转录回退，重复导入不增加单集，完整云端文本不被覆盖。
  之所以这样改：个人研究系统已经完成部分转录，公开服务可复用这些原始材料，减少重复工作，同时不公开私人研究报告。

### 修复

- a16z 改为标题明确匹配 AI 专题才收录，在缓存复用和转录前过滤；避免比特币、泛创业、毕业演讲及简介通用 AI 宣传混入。

- 收件目录更新触发仅导入的工作流；云端任务串行运行，Mac mini 与云端不直接写同一份播客列表。单集导入不把云端扫描时间刷新为当前时间。

### 文档

- 新增 [本地播客复用试点](docs/local-podcast-pilot.md)，说明发布范围、验证、两周观察与回退方式。公开摘要沿用用户 Agent 生成的现有方式。
